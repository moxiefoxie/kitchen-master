...
export default function HomeClient({ initialLocation, playIntro }: { initialLocation?: string; playIntro: boolean }) {
  const [menuOpen, setMenuOpen] = useState(false);
  const [locationState, setLocationState] = useState<"idle" | "loading" | "found" | "denied">("idle");
  const [miles, setMiles] = useState<number | null>(null);
  const [selectedId, setSelectedId] = useState(initialLocation || "suwanee");
  ...
  const [locationChosen, setLocationChosen] = useState(Boolean(initialLocation));
    ...
    <main>
      ...
      <section className="full-menu happy-hour-menu" id="happy-hour">
        <div className="happy-hour-grid">
          <div className="happy-hour-heading">
            <p className="kicker">{homePage.happyHourEyebrow}</p>
            <h2>{homePage.happyHourTitle}<br /><em>{homePage.happyHourAccent}</em></h2>
