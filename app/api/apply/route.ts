...
import { attachmentDigest, sendTransactionalEmail, submissionIdempotencyKey, transactionalEmailConfigured } from "@/lib/email";
import { applicationEmail } from "@/lib/emailTemplates";
...
export async function POST(request: Request) {
  ...
  const values = { name, email, phone, role, message };
  let delivered = false;
  if (transactionalEmailConfigured()) {
    const content = Buffer.from(await resume.arrayBuffer()).toString("base64");
    const emailMessage = applicationEmail({ locationName:location.name, values });
    const result = await sendTransactionalEmail({
      to: location.hiringEmail,
      replyTo: email,
      ...emailMessage,
      attachments: [{ filename:resume.name, content }],
      tags: [
        { name:"source", value:"website" },
        { name:"form", value:"careers" },
        { name:"location", value:location.slug },
      ],
      idempotencyKey: submissionIdempotencyKey("careers", {
        ...values,
        location:location.slug,
        resume:resume.name,
        resumeDigest:attachmentDigest(content),
      }),
    });
    delivered = result.ok;
  } else if (webhookUrl) {
  ...
  if (!delivered) return NextResponse.json({ error: "Application could not be submitted." }, { status: 502 });
